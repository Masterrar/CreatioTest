namespace Terrasoft.Configuration.FileImport
{
	...
	public class ProcessEntitiesFileImportStage : BaseFileImportStage
	{
		...
		private IList<ImportDataChunk<EntityChunkData>> GetEntitiesChunksData(ImportParameters parameters) {
			var existsEntitiesChunks = ImportEntitiesChunksDataProvider.Get(parameters.ImportSessionId).ToList();
			if (existsEntitiesChunks.Any()) {
				return existsEntitiesChunks;
			}
			var entitiesChunks = parameters.Entities.SplitOnChunks(parameters.ChunkSize);
			var newEntitiesChunks = new List<ImportDataChunk<EntityChunkData>>();
			foreach (var chunk in entitiesChunks) {
				newEntitiesChunks.Add(CreateEntityDataChunk(parameters.ImportSessionId, chunk));
			}
			ImportEntitiesChunksDataProvider.Add(newEntitiesChunks);
			return newEntitiesChunks;
		}

		private ConstructorArgument GetUserConnectionArgument() =>
				new ConstructorArgument("userConnection", UserConnection);

		private void InitCounts(uint totalRowsCount) {
		...
		private void NotifyListenersOnEntitySaved(uint processedRowsCount) {
			if (processedRowsCount % _onePercentValue > 0) {
				return;
			}
		...
		private void PopulateProcessedChunks(IEnumerable<ImportDataChunk<EntityChunkData>> entityChunks) {
			_processedWithErrorRowsCount = (uint)entityChunks.Sum(chunk => chunk.Data.ProcessedWithErrorEntityCount);
			_successProcessedRowsCount = (uint)entityChunks.Sum(chunk => chunk.Data.SuccessProcessedEntityCount);
		}
		...
		protected override FileImportStagesEnum? InternalProcess(ImportParameters parameters) {
			...
			InitCounts(parameters.TotalRowsCount);
			CreateTags(parameters.RootSchemaUId, parameters.ImportTags);
			var entityChunks = GetEntitiesChunksData(parameters);
			PopulateProcessedChunks(entityChunks);
			ProcessChunks(parameters, entityChunks);
			DeleteTags();
