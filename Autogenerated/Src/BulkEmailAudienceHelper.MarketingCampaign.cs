	...
	public class BulkEmailAudienceHelper {
		...
		virtual protected void ExecuteUpdateSplitTestAudience(Guid id, UserConnection userConnection) {
			...
			try {
				...
				//Temp fix to avoid bug with nolock hint (there are no way to disable it).
			string selectMaxId = string.Format("SELECT Max(Id) FROM {0}", transferTableName);
			using (DBExecutor dbExecutor = UserConnection.EnsureDBConnection()) {
				using (IDataReader reader = dbExecutor.ExecuteReader(selectMaxId)) {
