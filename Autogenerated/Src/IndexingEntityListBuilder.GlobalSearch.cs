namespace Terrasoft.Configuration.GlobalSearch
{
	...
	public abstract class IndexingEntityListBuilder
	{
		...
		protected virtual IgnoredEntityList IgnoredEntityList {
			get {
				return _ignoredEntityList ?? (_ignoredEntityList = new IgnoredEntityList());
			}
			set {
		...
		protected virtual EntitySchemaQueryFilterCollection ApplyModifiedOnFilters(EntitySchemaQuery esq) {
			...
			foreach (var column in entitySchema.Columns.Where(x => x.DataValueType.IsLookup && x.ReferenceSchema.ModifiedOnColumn != null)) {
				try {
					if (GetIsIndexedColumn(column, entitySchema)) {
						esqFilters.Add(esq.CreateFilterWithParameters(FilterComparisonType.Between,
							string.Format("{0}.{1}", column.Name, column.ReferenceSchema.ModifiedOnColumn.Name), _fromSqlDateTimeMacro, _toSqlDateTimeMacro));
					}
				} catch (Exception ex) {
					_log.Error(string.Format("Add lookup column {0} ModifiedOn filter. Error: {1}",
						column.Name, ex.Message));
				}
			}
