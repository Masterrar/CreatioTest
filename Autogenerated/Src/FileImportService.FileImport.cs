namespace Terrasoft.Configuration.FileImport
{
	...
	public class FileImportService : BaseService, IFileImportService, IReadOnlySessionState
	{
		...
		protected IBaseFileImporter FileImporter {
			get {
				if (_fileImporter == null) {
					...
					_fileImporter = UserConnection.GetIsFeatureEnabled("UsePersistentFileImport")
							? ClassFactory.Get<IBaseFileImporter>(userConnectionArg)
							: ClassFactory.Get<IFileImporter>(userConnectionArg);
				}
		...
		private void StartFileImportProcess(Guid importSessionId) {
			...
			UserConnection.RunProcess(FileImportProcessName, MisfireInstruction.SimpleTrigger.FireNow, parameters, jobOptions);
		}
	...
	public class ImportSessionInfoResponse : ConfigurationServiceResponse
	{
		...
		public void SetImportSessionInfo(ImportParameters parameters) {
			...
			NotImportedRowsCount = parameters.NotImportedRowsCount;
