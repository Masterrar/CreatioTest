namespace Terrasoft.Configuration.ExternalAccessPackage
{
	...
	public class TempAccessService : BaseService, System.Web.SessionState.IReadOnlySessionState
	{
		...
		private ITempAccessProxy GetTempAccessProxy(UserConnection userConnection) {
			...
			var tempAccessProxy = ClassFactory.Get<ITempAccessProxy>(proxyParams);
		...
		public List<GetTempAccessListDto> GetTempAccessList(string[] customerIds) {
			CheckOperationRights();
			var accessProxy = GetTempAccessProxy(UserConnection);
			List<(string AccessId, string Url, string Description, DateTime ExpirationDate, string CustomerId)> result
				= accessProxy.GetAccessList(customerIds);
			return result
				.Where(item => item.ExpirationDate.ToUniversalTime() >= DateTime.UtcNow)
