	...
	public class EntityInForecastCellRepository : IForecastCellRepository
	{
		...
		private string GetEntityColumnName() {
			...
			EntitySchemaColumn column = forecastEntityInCell.Columns.FirstOrDefault(c =>
				c.ReferenceSchemaUId == ForecastSheet.ForecastEntityUId && !c.IsInherited);
			return column?.Name;
		}
		...
		private void ChunkSave(IEnumerable<Cell> cells) {
			...
			cells = GetAllowedCellToInceset(cells);
		...
		private IEnumerable<Cell> GetAllowedCellToInceset(IEnumerable<Cell> cells) {
			return cells
