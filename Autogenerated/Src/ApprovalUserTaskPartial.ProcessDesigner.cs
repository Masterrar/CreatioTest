namespace Terrasoft.Core.Process.Configuration
{
	...
	public partial class ApprovalUserTask
	{
		...
		protected ApprovalUserTaskMacrosHelper MacrosHelper {
			get {
				return _macrosHelper ?? new ApprovalUserTaskMacrosHelper {
		...
		private Guid GetEmployeeId() {
			var selectById =
				new Select(UserConnection).Top(1)
					.Column("Id")
				.From("SysAdminUnit")
				.Where("Id").IsEqual(Column.Parameter(EmployeeId)) as Select;
			...
			var selectByContactId =
				new Select(UserConnection).Top(1)
					.Column("Id")
				.From("SysAdminUnit")
				.Where("ContactId").IsEqual(Column.Parameter(EmployeeId)) as Select;
		...
		private Guid GetEmployeeManagerId() {
			var selectById =
				...
				.Where("se", "Id").IsEqual(Column.Parameter(EmployeeId)) as Select;
			Guid employeeId = selectById.ExecuteScalar<Guid>();
			if (employeeId.IsNotEmpty()) {
				return employeeId;
			}
			var selectByContactId =
				...
				.InnerJoin("Employee").As("m").On("m", "ContactId").IsEqual("sm", "ContactId")
				.InnerJoin("Employee").As("e").On("e", "ManagerId").IsEqual("m", "Id")
				.Where("e", "ContactId").IsEqual(Column.Parameter(EmployeeId)) as Select;
			employeeId = selectByContactId.ExecuteScalar<Guid>();
			return employeeId;
		}

		private Guid GetVisaOwnerId() {
			var visaOwnerId = Guid.Empty;
		...
		private DataContract.Filter CreateStatusFilter(EntitySchema schema, Guid statusId) {
		...
		private string GetConditionData(EntitySchema schema) {
			...
			select.ExecuteReader(reader => {
				...
				DataContract.Filter filter = CreateStatusFilter(schema, statusId);
		...
		private EmailMessage CreateEmailMessage(List<string> recepients, Guid emailTemplateId) {
		...
		private string GetSenderEmailAddress() {
			string senderEmailAdress = string.Empty;
			if (CoreSysSettings.TryGetValue(UserConnection, "VisaMailboxSettings", out object value)) {
				...
				Select select = new Select(UserConnection)
					...
					.Where("MailboxSyncSettings", "Id").IsEqual(Column.Parameter(mailboxSettingsId)) as Select;
				senderEmailAdress = select.ExecuteScalar<String>();
			}
			if (senderEmailAdress.IsNullOrEmpty()) {
		...
		private List<string> GetAuthorEmailAddresses() {
			...
			var parameter = parameters.FindByName("AuthorEmailAddress");
			if (parameter != null) {
				object propertyValue = GetParameterValue(parameter);
				result = EmailTemplateUserTaskUtilities.GetParameterEmailAddresses(UserConnection, propertyValue, parameter);
			}
			return result;
		}
		...
		private Select GetEmployeeContactIdSelect(Guid employeeId) {
			var select =
				...
				.From("Contact")
				.InnerJoin("SysAdminUnit").On("Contact", "Id").IsEqual("SysAdminUnit", "ContactId")
				.Where("SysAdminUnit", "Id").IsEqual(Column.Parameter(employeeId)) as Select;
			return select;
		}
		...
		protected override bool InternalExecute(ProcessExecutingContext context) {
			...
			if (!isRunning || isError) {
				...
				string conditionData = GetConditionData(schema);
				...
				if (IsSendEmailToApprovers) {
					List<string> recepients = GetApproversEmailAddresses();
					if (recepients.IsNotEmpty()) {
						SendEmail(recepients, ApproverEmailTemplate);
					}
				}
			}
			return false;
		...
		public override void CancelExecuting(params object[] parameters) {
			...
			UserConnection.IProcessEngine.RemoveActivityProcessListener(CurrentApprovalId, UId,
