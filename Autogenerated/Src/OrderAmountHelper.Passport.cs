...
namespace Terrasoft.Configuration.Passport
{
	public class OrderAmountHelper
	{
		...
		protected class UpdateAmountsInfo
		{
			...
			public decimal OrderProductToPrimaryFactor {
				get { return OrderProductCurrencyInfo.ToPrimaryFactor; }
			}

			public decimal OrderToPrimaryFactor {
				get { return GetToPrimaryFactor(OrderCurrencyDivision, OrderCurrencyRate); }
			}
		}
		...
		protected class CurrencyInfo
		{
			...
			public decimal ToPrimaryFactor {
				get { return GetToPrimaryFactor(Division, Rate); }
			}

			public decimal FromPrimaryFactor {
				get { return GetToPrimaryFactor(Rate, Division); }
			}
		}
		...
		protected virtual Update GetInvoiceProductUpdate(UpdateAmountsInfo updateAmountsParams) {
			var factor = updateAmountsParams.OrderProductCurrencyInfo.ToPrimaryFactor;
			if (factor == 0) {
				factor = GetToPrimaryFactor(updateAmountsParams.OrderCurrencyDivision, updateAmountsParams.OrderCurrencyRate);
			}
			...
			Update update = (Update)(new Update(UserConnection, "InvoiceProduct").WithHints(Hints.RowLock)
				...
				.Set("Price", Column.Parameter(updateAmountsParams.OrderProductPrice))
				.Set("Amount", productQuantityExpression * Column.Parameter(updateAmountsParams.OrderProductPrice))
				.Set("DiscountPercent", Column.Parameter(updateAmountsParams.OrderProductDiscountPercent))
				.Set("DiscountAmount", discountAmountExpression)
				.Set("TaxId", GetGuidColumnExpression(updateAmountsParams.OrderProductTaxId))
				.Set("DiscountTax", Column.Parameter(updateAmountsParams.OrderProductDiscountTax))
				.Set("TaxAmount", taxAmountExpression)
				.Set("TotalAmount", productAmountExpression)
				.Set("PrimaryTotalAmount", productAmountExpression * Column.Parameter(factor))
				.Set("PrimaryTaxAmount", taxAmountExpression * Column.Parameter(factor))
				.Set("PrimaryPrice", Column.Parameter(updateAmountsParams.OrderProductPrice) * Column.Parameter(factor))
				.Set("PrimaryAmount",
					productQuantityExpression * Column.Parameter(updateAmountsParams.OrderProductPrice) * Column.Parameter(factor))
				.Set("PrimaryDiscountAmount", discountAmountExpression * Column.Parameter(factor))
				.Where().Exists(
		...
		protected virtual Update GetInvoiceAmountUpdate(CurrencyInfo currencyInfo) {
			var update = new Update(UserConnection, "Invoice")
				...
				.Set("CurrencyId", GetGuidColumnExpression(currencyInfo.Id))
				.Set("CurrencyRate", Column.Parameter(currencyInfo.Rate))
				.Set("Amount",
					Func.Coalesce(GetInvoiceProductSelectByInvoice("TotalAmount"), GetSupplyPaymentSelectByInvoice("AmountPlan"),
						Column.Const(0)))
				.Set("PrimaryAmount",
					Func.Coalesce(GetInvoiceProductSelectByInvoice("PrimaryTotalAmount"),
						GetSupplyPaymentSelectByInvoice("PrimaryAmountPlan"), Column.Const(0)))
				.Set("PaymentAmount",
					Column.SourceColumn("PrimaryPaymentAmount") * Column.Parameter(currencyInfo.FromPrimaryFactor));
			return update;
		}
		...
		/// <summary>
		/// Updates invoice ammount, related to <paremref name="supplyPaymentElementId"/>.
		/// </summary>
		...
		/// <param name="supplyPaymentElementId">Id ####.</param>
		...
		/// <param name="elementTypeId">Id #### #### ####### ######## # #####.</param>
		...
		/// </summary>
		/// <param name="invoiceId">Id #####.</param>
		/// <param name="changedColumnValues">########## ######## #######.</param>
		/// <param name="orderPaymentAmountUpdated">True, #### ##### ###### ###### #### #########.</param>
		...
		/// </summary>
		/// <param name="orderId">Id ######.</param>
		/// <param name="supplyPaymentElementIds">###### id #####.</param>
		...
		/// </summary>
		/// <param name="orderId">Id ######.</param>
		/// <param name="supplyPaymentElementList">###### #####.</param>
