	...
	public class GCalendarSynchronizer : BaseGoogleSynchronizer
	{
		...
		private void ProcessDeletedInGoogleActivity(GoogleActivity gActivity) {
			...
			using (DBExecutor dbExecutor = UserConnection.EnsureDBConnection()) {
				using (IDataReader dataReader = dbExecutor.ExecuteReader(sel.GetSqlText(), sel.Parameters)) {
					if (dataReader.Read()) {
		...
		private EntitySchemaQuery GetExistingActivitiesQuery(IEnumerable<SyncEntity<GoogleActivity>> googleActivities,
			...
			IEnumerable<string> extIds = googleActivities.Select(a => a.Item.ExtId);
		...
		public override void ProcessBPMonlineChangedEntities() {
			...
			var correspondenceModifiedOnFilterGroup = new EntitySchemaQueryFilterCollection(esq, LogicalOperationStrict.And) {
				esq.CreateFilter(FilterComparisonType.Less, "[ActivityCorrespondence:Activity].ModifiedOn",
			...
			modifiedOnFilterGroup.Add(correspondenceModifiedOnFilterGroup);
