	...
	public class CampaignHelper
	{
		...
		private static Select GetImmediatelyTriggerBulkEmailIdentifierByStepIdSelect(
			...
			var selectStepBulkEmailId = new Select(userConnection)
				...
				.And("TypeId").IsEqual(Column.Const(CampaignConsts.BulkEmailCampaignStepTypeId));
			var select = new Select(userConnection)
				...
				.And("StatusId").IsEqual(Column.Const(MarketingConsts.BulkEmailStatusActiveId))
				.And("LaunchOptionId")
				.IsEqual(Column.Const(MarketingConsts.TriggerEmailImmediateLaunchOptionId)) as Select;
			return select;
		}
		...
		private static Select GetCampaignListByLandingIdSelect(UserConnection userConnection, Guid landingId) {
			var select = new Select(userConnection)
				...
				.Where("C", "CampaignStatusId").IsEqual(Column.Const(CampaignConsts.RunCampaignStatusId))
				.And("CS", "TypeId").IsEqual(Column.Const(CampaignConsts.LendingCampaignStepTypeId))
