namespace Terrasoft.Configuration.SSP
{
	...
	public class SspUserInviter
	{
		...
		private void ActivateUsers() {
			var usersToConnect = UserInvites.Where(x => x.SysAdminUnitId != Guid.Empty);
			SspUserAdministrator.ActivateUsersByIds(usersToConnect.Select(user => user.SysAdminUnitId).ToList());
		}
