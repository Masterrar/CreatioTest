namespace Terrasoft.Configuration.ReportService
{
	...
	public class ReportService : BaseService
	{
		...
		private Report GetDevExpressReport(UserConnection userConnection, string reportSchemaUId, object id,
				string entitySchemaUId, Dictionary<string, object> filterParameters) {
			var manager = userConnection.GetSchemaManager("ReportSchemaManager") as ReportSchemaManager;
			ReportSchema reportSchema = manager.FindInstanceByUId(new Guid(reportSchemaUId));
			if (reportSchema == null) {
				return null;
			}
			reportSchema.InitializeLocalizableValues();
			Report report = reportSchema.CreateReport(userConnection);
			report.Caption = reportSchema.Caption.ToString();
			if (report.Parameters.Count == 0 || filterParameters == null) {
				return report;
			}
			foreach (var param in filterParameters) {
				if (report.Parameters[param.Key] != null && param.Key != "Filters") {
					report.Parameters[param.Key].Value = param.Value;
				} else {
					if (param.Value != null) {
						var filters = Json.Deserialize<Terrasoft.Nui.ServiceModel.DataContract.Filters>(param.Value.ToString());
						var esqFilters = filters.BuildEsqFilter(new Guid(entitySchemaUId), userConnection);
						if (report.Parameters[param.Key] != null) {
							report.Parameters[param.Key].Value = esqFilters;
						}
					}
				}
			}
		...
		public ReportData GenerateDevExpressReport(string entitySchemaUId, string reportSchemaId, string recordId,
			...
			if (!GlobalAppSettings.UseReportGenerationBasedOnSchema) {
				...
				return reportGenerator.Generate(UserConnection, configuration);
			} else {
				Dictionary<string, object> parameters = null;
				if (!string.IsNullOrEmpty(reportParameters)) {
					parameters = JsonConvert.DeserializeObject<Dictionary<string, object>>(reportParameters);
				}
				Report report = GetDevExpressReport(UserConnection, reportSchemaId, recordId, entitySchemaUId, parameters);
				...
				var result = new ReportData {
					...
					Format = "pdf",
					Data = reportStream.ReadAllBytes()
				};
				return result;
			}
		}
		...
		private WordprocessingDocument FillTableFieldsOutsideTables(WordprocessingDocument docx, Guid recordId,
			...
			foreach (var macrosCaption in dic) {
				...
				EntityCollection entityCollection = GetEntityValuesByColumnName(recordId, schemaUId,
					...
					referenceColumnCaption, additionalMacros, schemaName, baseFilterPath);
		...
		private void FillTableUsingMacrosData(Guid recordId, string schemaName, WordprocessingDocument docx,
			...
			foreach (var macrosDataItem in additionalMacrosesData) {
				...
				EntityCollection entityCollection = GetEntityValuesByColumnName(recordId,
					...
					macrosDataItem.Value.ObjectColumnPath);
