namespace Terrasoft.Configuration.FileImport
{
	...
	public abstract class BaseFileImporter : IBaseFileImporter
	{
		...
		private ImportParameters ForceGetImportParameters(Guid importSessionId) {
		...
		protected void SaveImportParameters(ImportParameters parameters) => parameters.Save();
		...
		public ColumnsMappingResponse GetColumnsMappingParameters(Guid importSessionId) {
			importSessionId.CheckArgumentEmpty(nameof(importSessionId));
		...
		public void SetFileInfo(Guid importSessionId, string fileName) {
			importSessionId.CheckArgumentEmpty(nameof(importSessionId));
			fileName.CheckArgumentNullOrWhiteSpace(nameof(fileName));
		...
		/// <inheritdoc />
		public void SetImportObject(Guid importSessionId, ImportObject importObject) {
			importSessionId.CheckArgumentEmpty(nameof(importSessionId));
			...
			if (parameters.HeaderColumnsValues != null) {
				var fileProcessor = CreateFileProcessor(parameters);
				fileProcessor.ProcessObject(importObject.UId);
			}
			SaveImportParameters(parameters);
		}
