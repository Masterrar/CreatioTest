...
namespace Terrasoft.Configuration
{
	...
	public class FolderHelper
	{
		...
		private FolderInfoModel GetFolderInfoModel(string folderSchemaName, Guid folderId,
			...
			var contactFolderEntity = esq.GetEntity(userConnection, folderId);
			if (contactFolderEntity == null) {
				return null;
			}
			string searchDataStr = string.Empty;
			byte[] searchData = contactFolderEntity.GetBytesValue("SearchData");
			if (searchData != null && searchData.Length != 0) {
				searchDataStr = Encoding.UTF8.GetString(searchData);
			}
			return new FolderInfoModel {
				Id = folderId,
				DisplayValue = contactFolderEntity.GetTypedColumnValue<string>("Name"),
