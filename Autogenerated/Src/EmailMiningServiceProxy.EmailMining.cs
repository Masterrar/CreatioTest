namespace Terrasoft.Configuration.EmailMining
{
	...
	using Terrasoft.Configuration.EnrichmentDto;
	...
	public class EmailMiningServiceProxy : IEmailMiningServiceProxy
	{
		...
		public EmailMiningServiceProxy(UserConnection userConnection) {
			...
			if (!ClassFactory.TryGet("EmailMiningServiceProxyRestClient", out _restClient)) {
		...
		private IRestRequest PrepareBatchRequest(GetEmailEntitiesRequest[] requests) {
			var batchRequest = new RestRequest {
				Method = Method.POST,
				RequestFormat = DataFormat.Json,
				Resource = ServiceEmailParsingMethod,
				JsonSerializer = new NewtonsoftBasedSerializer()
			};
			batchRequest.AddBody(requests);
			return batchRequest;
		}
		...
		public Dictionary<Entity, GetEmailEntitiesResponse> BatchCall(IEnumerable<Entity> emails) {
			...
			string serviceUrl;
			string apiKey;
			CheckRequiredParameters(out serviceUrl, out apiKey);
			_restClient.BaseUrl = serviceUrl;
			_restClient.Timeout = (int)_requestTimeout.TotalMilliseconds;
			...
			IRestRequest batchRequest = PrepareBatchRequest(requests);
			IRestResponse batchResponse = _restClient.Post(batchRequest);
