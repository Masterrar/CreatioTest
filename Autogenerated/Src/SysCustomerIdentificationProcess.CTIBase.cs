namespace Terrasoft.Core.Process
{
	...
	public class SysCustomerIdentificationProcess : Terrasoft.Core.Process.Process
	{
		...
		private bool ConditionalFlow1ExpressionExecute() {
			return Convert.ToBoolean((CallerReverse).Length >= (SearchNumberLength));
		}

		private bool ConditionalFlow2ExpressionExecute() {
			return Convert.ToBoolean((CallerReverse).Length <= (InternalNumberLength));
		}

		private bool ConditionalFlow3ExpressionExecute() {
			return Convert.ToBoolean((IsManualSearch));
		}

		private bool ConditionalFlow4ExpressionExecute() {
			return Convert.ToBoolean((SearchName).Length >= 3);
		}
