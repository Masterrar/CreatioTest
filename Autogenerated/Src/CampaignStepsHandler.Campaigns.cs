namespace Terrasoft.Configuration
{
	...
	public partial class CampaignStepsHandler
	{
		...
		private CampaignNextStepPriority GetCampaignNextStepInfo(Guid sourceStepId) {
			...
			var selectFirstStepRoute = new Select(_userConnection)
				.Top(1)
				.Column("TargetStepId")
				.Column("JSON")
				.From("CampaignStepRoute")
			...
			using (DBExecutor dbExecutor = _userConnection.EnsureDBConnection()) {
				using (IDataReader reader = dbExecutor.ExecuteReader(selectFirstStepRoute.GetSqlText(),
					selectFirstStepRoute.Parameters)) {
					if (reader.Read()) {
		...
		private void ActivateReferencedCampaignEmails(Guid campaignId, Guid stepId) {
			try {
				...
				var updateStatusId = new Update(_userConnection, "BulkEmail").WithHints(new RowLockHint())
					...
					.Where("Id").In(select)
					.And("LaunchOptionId").IsEqual(Column.Const(MarketingConsts.TriggerEmailFromCampaignLaunchOptionId))
					.And("StatusId").In(
		...
		private Dictionary<Guid, Guid> GetCampaingsStepsForActualize(IList<Guid> campaignList = null) {
			...
			Query selectQuery = new Select(_userConnection)
			...
			using (DBExecutor dbExecutor = _userConnection.EnsureDBConnection()) {
				using (IDataReader reader = dbExecutor.ExecuteReader((selectQuery as Select).GetSqlText(),
					selectQuery.Parameters)) {
					while (reader.Read()) {
