namespace Terrasoft.Configuration.GlobalSearch
{
	...
	public class IgnoredEntityList
	{
		...
		public bool GetIsIgnoredColumn(EntitySchema entitySchema, EntitySchemaColumn column) {
			string columnAlias = GlobalSearchColumnUtils.GetAlias(column, entitySchema);
			string columnPath = string.Format("{0}.{1}", entitySchema.Name, column.Name);
		...
		public bool GetIsIgnoredSection(EntitySchema entitySchema) {
			return entitySchema == null 
				|| IgnoredSections.Contains(entitySchema.Name) 
