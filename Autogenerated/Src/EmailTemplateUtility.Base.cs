namespace Terrasoft.Configuration
{
	...
	public class EmailTemplateExecutor
	{
		...
		/*private string RelationColumnName {
			get {
				if (EmailTemplateEntity == null)
					return string.Empty;
				var objectFieldInActivityId = EmailTemplateEntity.GetTypedColumnValue<Guid>("ObjectFieldInActivity");
				if (objectFieldInActivityId == Guid.Empty) 
					return string.Empty;
				var activitySchema = UserConnection.EntitySchemaManager.GetInstanceByName("Activity");
				var relationColumn = activitySchema.Columns.FindByUId(objectFieldInActivityId);
				return relationColumn.ColumnValueName;
			}
		}*/
		...
		private Dictionary<Guid, Tuple<byte[], string>> GetAttachments(Guid emailTemplateId) {
			...
			var idQueryColumn = esq.AddColumn("Id");
		...
		public void SendEmail() {
			...
			var attachments = GetAttachments(EmailTemplateId);
			var client = new Terrasoft.Mail.SmtpClient(UserConnection);
			if (string.IsNullOrEmpty(SendFromAddress) && !TryCreateSender())
				return;
#if !NETSTANDARD2_0 // TODO #CRM-44355
			var sender = Terrasoft.UI.WebControls.Utilities.ControlUtilities.HtmlDecode(SendFromAddress);
			var splitSender = sender.Split(new Char[] { '<', '>' });
			var credential = client.CreateMailCredentials(splitSender[1]);
#else
			MailCredentials credential = null;
#endif
			...
			if (SendIndividualEmail) {
				foreach (var mailingContact in mailingCollection) {
					...
					Guid contactId = EmailRecipients[mailingContact];
					ErrorCode = string.Empty;

					string[] recipients = null;
					if (recepient != null && recepient != String.Empty) {
						recipients = new string[] { recepient };
					}
					MailMessage newMessage = null;
					try {
						newMessage = client.CreateMessage(
							EmailTemplateUtility.ReplaceRecipientMacrosText(body, contactId, macrosList, UserConnection),
							EmailTemplateUtility.ReplaceRecipientMacrosText(subject, contactId, macrosList, UserConnection),
							recipients,
							SendFromAddress,
							isHtmlBody,
							attachments,
					...
					if (string.IsNullOrEmpty(ErrorCode)) {
						try {
							client.SendMessage(newMessage, credential);
							_emailsWasSended++;
						} catch (Exception e) {
							ErrorCode = e.Message;
						} finally {
				...
				string[] recipients = new string[mailingCollection.Count];
				for (int i = 0; i < mailingCollection.Count; i++)
					recipients[i] = mailingCollection[i].Split(new Char[] { '<', '>' })[1];
