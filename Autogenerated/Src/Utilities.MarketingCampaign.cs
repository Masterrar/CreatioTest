namespace Terrasoft.Configuration.Marketing.Utilities
{
		...
		public void ExecuteInsert() {
			if (_counter > 0) {
				using (var dbExecutor = _userConnection.EnsureDBConnection()) {
					dbExecutor.Execute(_queryBuilder.ToString());
				}
				_counter = 0;
				_queryBuilder.Clear();
			}
	...
	public class TypedCounter<T>
	{
		...
		private readonly Dictionary<string, InnerCounter<T>> _counters;
		...
		/// </summary>
		public TypedCounter() {
			_counters = new Dictionary<string, InnerCounter<T>>();
		}
		...
		public void Count(T arg) {
			foreach (InnerCounter<T> counter in _counters.Values) {
				counter.Count(arg);
			}
		}
