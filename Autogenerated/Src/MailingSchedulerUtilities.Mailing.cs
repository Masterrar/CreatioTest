namespace Terrasoft.Configuration {
	...
	public static class MailingSchedulerUtilities {
		...
		private static readonly Guid SystemSysAdminUnitId = new Guid("7F3B869F-34F3-4F20-AB4D-7480A5FDF647");
		...
		private static string GetJobExecutorUserName(UserConnection userConnection) {
			string resultName = userConnection.CurrentUser.Name;
			if (userConnection.CurrentUser.Id != SystemSysAdminUnitId) {
				return resultName;
			}
			var selectQuery = new Select(userConnection)
				...
				.From("SysAdminUnit")
				.Where("Id")
				.IsEqual(Column.Parameter(SystemSysAdminUnitId)) as Select;
			using (DBExecutor dbExecutor = userConnection.EnsureDBConnection()) {
				using (IDataReader reader = selectQuery.ExecuteReader(dbExecutor)) {
					if (reader.Read()) {
						resultName = reader.GetColumnValue<string>("Name");
					}
				}
			}
			return resultName;
