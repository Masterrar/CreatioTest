...
define("ServiceModelDiagramViewModel", ["ext-base", "ServiceHelper", "ServiceModelDiagramViewModelResources",
	...
	function(Ext, ServiceHelper, resources) {
		Ext.define("Terrasoft.configuration.ServiceModelDiagramViewModel", {
			...
			CONNECTOR_WIDTH: 1,
			CONNECTOR_SEGMENTS_TYPE: ej.datavisualization.Diagram.Segments.Straight,
			...
			updateCachedConfigs: function(response) {
				document.getElementById(this.DIAGRAM_CONTAINER).focus();
				var mainElement = response.mainElement;

				var influenceElements = mainElement.influenceElements;
				var dependenceElements = mainElement.dependenceElements;
			...
			addLabelForEmptyColumn: function(elements, caption, side, collectionName) {
				...
				labelConfig.textAlign = ej.datavisualization.Diagram.TextAlign.Center;
			...
			buildDiagramNodes: function(response) {
				...
				var cache = this.Ext.create("Terrasoft.Collection");
				var nodes = this.get("Nodes");
				for (var i = 0; i < nodes.collection.items.length; i++) {
			...
			rebuildDiagramNodes: function(collection) {
				...
				var influenceElements = [];
				var dependenceElements = [];
