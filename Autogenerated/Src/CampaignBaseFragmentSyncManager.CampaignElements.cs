namespace Terrasoft.Configuration
{
	...
	public abstract class CampaignBaseFragmentSyncManager : ICampaignFragmentSyncManager
	{
		...
		protected const string MandrillRecipientTableName = "MandrillRecipient";
		...
		private int GetCampaignElementAudienceQueryBatchSize(UserConnection userConnection) {
		...
		private void MoveRelations(Select relationsSelect, short wasUsed) {
		...
		private Select GetParticipantsToSyncSelect(Select participantsToSyncSelect) {
			...
			campaignParticipantOpToSyncSelect
				.Column(Func.Max("Id")).As("Id")
				.GroupBy("ContactId");
			campaignParticipantOpToSyncSelect.SpecifyNoLockHints();
			return campaignParticipantOpToSyncSelect;
		}
		...
		protected virtual Select GetParticipantsToSyncQuery(UserConnection userConnection) {
			var select = new Select(userConnection)
				...
				.Where(CampaignParticipantOpTableName, "IsReadyToSync").IsEqual(Column.Const(true))
					.And(CampaignParticipantOpTableName, "InitialCampaignItemId")
						.IsEqual(Column.Parameter(CampaignItemId)) as Select;
		...
		/// <summary>
		/// Returns True if there are some capmaign participants to synchronize.
		/// Otherwise returns False.
		/// </summary>
		/// <returns>Is there any campaign participant to sync.</returns>
		protected virtual bool HasParticipantsToSync() {
		...
		protected virtual Insert GetParticipantToRecipientRelationsInsert(UserConnection userConnection,
				IEnumerable<(Guid, Guid)> relations, short wasUsed) {
			...
			foreach (var relation in relations) {
				query.Values()
					...
					.Set("WasUsed", Column.Const(wasUsed));
			}
			return query;
		}
		...
		protected virtual void SyncParticipantRelations(UserConnection userConnection) {
			var isRecurringFeatureEnabled = userConnection.GetIsFeatureEnabled("RecurringCampaigns");
			if (!isRecurringFeatureEnabled) {
				return;
			}
			var actualRelationsSelect = GetOperationalRecipientRelationsToMoveSelect();
			MoveRelations(actualRelationsSelect, wasUsed: 0);
			var usedRelationsSelect = GetUsedOperationalRecipientRelationsToMoveSelect();
			MoveRelations(usedRelationsSelect, wasUsed: 1);
		}
		...
		public void Synchronize(UserConnection userConnection, ref int synchronizedCount) {
			...
			try {
				var useCampaignBatchedQueries = userConnection.GetIsFeatureEnabled("CampaignBatchedQueries");
				if (useCampaignBatchedQueries) {
					BatchedExecute(userConnection, ref synchronizedCount);
				} else {
					if (!HasParticipantsToSync()) {
						return;
					}
					synchronizedCount += InternalSynchronize(userConnection);
				}
