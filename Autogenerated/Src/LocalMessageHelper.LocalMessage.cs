...
namespace Terrasoft.Configuration
{
	...
	public class LocalMessageHelper
	{
		...
		private string CreateLink(string columnName) {
			...
			_entity.LoadLookupDisplayValues(columnName);
			var columnValue = _entity.GetTypedColumnValue<Guid>(entitySchemaColumn.ColumnValueName);
			var displayColumnValue = _entity.GetTypedColumnValue<string>(entitySchemaColumn.DisplayColumnValueName);
		...
		private string GetDisplayColumnValue(string columnName) {
			...
			return _entity.GetTypedColumnValue<string>(column.DisplayColumnValueName);
		}
		...
		private IEnumerable<string> GetChangedDisplayValues(string columnName) {
			...
			var oldValue = _entity.GetTypedOldColumnValue<string>(column.DisplayColumnValueName);
			list.Add(!string.IsNullOrEmpty(oldValue) ? oldValue : Empty);
			if (column.IsLookupType) {
				_entity.LoadLookupDisplayValues(columnName);
			}
			var value = _entity.GetTypedColumnValue<string>(column.DisplayColumnValueName);
	...
	public static class LocalMessageStore
	{
		...
		public static IList<LocalMessageStartEvent> GetLocalMessageStartEvents(UserConnection userConnection) {
			...
			var localMessageStartEvents = _localMessageStartEvents;
			...
			if (!localMessageStartEvents.ContainsKey(cultureId)) {
				...
				_localMessageStartEvents = localMessageStartEvents;
			}
		...
		public static IList<LocalMessageColumnTemplate> GetLocalMessageColumnTemplates(UserConnection userConnection) {
			var localMessageColumnTemplates = _localMessageColumnTemplates;
			...
			_localMessageColumnTemplates = localMessageColumnTemplates;
			return localMessageColumnTemplates;
		}
