namespace Terrasoft.Configuration
{
	...
	public class CampaignQueueManager : ICampaignQueueManager
	{
		...
		private void SetCampaignParticipantIdParameterValue(Insert insert, CampaignQueueItem item) {
			if (item.CampaignParticipantId.Equals(default(Guid))) {
				insert.Parameters.GetByName(CampaignParticipantIdColumnName).Value = null;
			} else {
				insert.Parameters.GetByName(CampaignParticipantIdColumnName).Value = item.CampaignParticipantId;
			}
		}
		...
		public int Enqueue(IEnumerable<CampaignQueueItem> items) {
			...
			foreach (var item in items) {
				...
				SetCampaignParticipantIdParameterValue(insert, item);
