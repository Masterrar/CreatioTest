        ...
        protected override bool InsertMessage() {
            ...
            try {
                if (insertResult && base.MessageNotifierId == _portalNotifierId) {
                    ...
                    _log.InfoFormat("[CaseId: {2}] CasePortalMessageMultiLanguageProcess should be started for Message: {0}, RecordId: {1}", messageInfo.Message, recordId, messageInfo.ListenersData[ListenerSchemaUId]);
                    RunProcess(recordId);
                }
            } catch (Exception ex) {
                _log.ErrorFormat("[CaseId: {3}] An error occured while starting process Message: {0}, CallStack: {1}, InnerException: {2}", ex.Message, ex.StackTrace, ex.InnerException, messageInfo.ListenersData[ListenerSchemaUId]);
            }
            return insertResult;
        }
