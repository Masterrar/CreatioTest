namespace Terrasoft.Configuration
{
	...
	public class ActiveContactsHelper
	{
		...
		/// </summary>
		private static readonly string[] _marketingActiveContactsLicensePackageNames =
			new[] { "marketing creatio active contacts", "marketing creatio 1000 active contacts", "bpmonline marketing active contacts" };
		...
		private static string GetActualLicensePackageName(UserConnection userConnection) {
			foreach (var packageName in _marketingActiveContactsLicensePackageNames) {
				var dueDate = GetLicenseDueDate(userConnection, packageName);
